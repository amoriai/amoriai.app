...
const LABELS: Record<Locale, Labels> = {
  ...
  en: {
    ...
    billingNote:
      "Secure billing via Stripe · Change or cancel anytime · No hidden fees",
  ...
  es: {
    ...
    billingNote:
      "Pago seguro con Stripe · Cambia o cancela cuando quieras · Sin cargos ocultos",
...
export default function PricingPage() {
    ...
    async function loadPlans() {
      ...
      if (error) {
        ...
        if (!cancelled) {
          setErrorMsg(
            locale === "fr"
              ? "Impossible de charger les prix (Supabase)."
              : "Unable to load pricing (Supabase)."
          );
  ...
  const startStripeCheckout = async (planId: PaidPlanId) => {
    ...
    if (userErr) {
      ...
      throw new Error(
        locale === "fr"
          ? "Erreur auth. Réessaie de te reconnecter."
