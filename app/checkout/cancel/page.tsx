...
const STRINGS: Record<
  Locale,
  { title: string; text: string; ctaPricing: string; ctaHome: string }
> = {
  fr: {
    title: "Paiement annulé",
    text: "Aucun montant n’a été prélevé. Tu peux réessayer quand tu veux.",
    ctaPricing: "Retour aux abonnements",
    ctaHome: "Accueil",
  },
  en: {
    title: "Payment canceled",
    text: "No charge was made. You can try again anytime.",
...
export default async function CancelPage({
  ...
  const pricingHref = `/pricing?lang=${encodeURIComponent(lang)}`;
    ...
    <main style={styles.page}>
      <section style={styles.card}>
        ...
        <div style={styles.row}>
          <Link href={pricingHref} style={{ ...styles.btn, ...styles.btnPrimary }}>
            {t.ctaPricing}
          </Link>
...
const styles: Record<string, React.CSSProperties> = {
  page: {
    ...
    background:
      "radial-gradient(1100px 700px at 50% -10%, rgba(56,189,248,0.16), transparent 60%)," +
      "radial-gradient(900px 700px at 10% 10%, rgba(249,115,22,0.12), transparent 55%)," +
